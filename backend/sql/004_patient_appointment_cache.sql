...
-- Columna generada: al menos una cita en estado activo (pending/confirmed/rescheduled)
do $$
begin
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'patients' and column_name = 'has_active_appointment'
  ) then
    alter table patients
...
comment on column patients.active_appointment_count is
  'Cantidad de citas con status pending|confirmed|rescheduled y deleted_at null.';
