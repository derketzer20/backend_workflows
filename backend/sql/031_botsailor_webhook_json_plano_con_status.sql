-- =============================================================================
-- 031 — BotSailor → Supabase (formato final array + status)
-- =============================================================================
-- Lo que arma BotSailor (y lo que procesa la función) es SOLO el array:
--
--   [
--     {
--       "numero": "5215548649518",
--       "tenant_id": "9e4860a5-d163-548d-8cb2-886f4d9e71f2",
--       "specialist_code": "dr_juan",
--       "status": "confirmado"
--     }
--   ]
--
-- Cancelar: mismo array con "status": "cancelado"
--
-- Si el POST va a PostgREST (URL .../rest/v1/rpc/fn_botsailor_appointment_response),
-- el parámetro de la función se llama p_payload; PostgREST exige envolver el array:
--
--   { "p_payload": [ { ... mismo objeto ... } ] }
--
-- Eso NO cambia el contenido: p_payload es exactamente el array de arriba.
-- En BotSailor: construir el array; en el nodo HTTP a Supabase, body = {"p_payload": <ese array>}.
--
-- Solo status confirmado|cancelado cambia la cita. Cualquier otro valor → ignored.
-- El recordatorio 4h/24h (fn_dispatch) NO llama esta función → no cambia status al enviar WhatsApp.
-- =============================================================================
...
create or replace function fn_botsailor_normalize_status(p_status text)
returns text
language plpgsql
immutable
set search_path = public
...
declare
  s text;
begin
  s := lower(btrim(coalesce(p_status, '')));
  if s in ('confirmado', 'confirmed', 'confirm', 'confirmada') then
    return 'confirmado';
  end if;
  if s in ('cancelado', 'cancelled', 'cancel', 'cancelada', 'rechazado', 'rechazada') then
    return 'cancelado';
  end if;
  return null;
end;
...
create or replace function fn_botsailor_appointment_response(p_payload jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if jsonb_typeof(p_payload) <> 'array' then
    return jsonb_build_object('ok', false, 'error', 'expected_json_array');
  end if;

  v_item := p_payload->0;
  if v_item is null then
    return jsonb_build_object('ok', false, 'error', 'array_empty');
  end if;

  v_status_in := v_item->>'status';
  v_status := fn_botsailor_normalize_status(v_status_in);

  if v_status is null then
    return jsonb_build_object(
      'ok', true,
      'action', 'ignored',
      'reason', 'status_debe_ser_confirmado_o_cancelado',
      'status_recibido', v_status_in,
      'hint', 'Solo llamar este webhook cuando el usuario pulse Confirmar o Cancelar en WhatsApp.'
    );
  end if;
  ...
  v_specialist_code := nullif(btrim(v_item->>'specialist_code'), '');
  if v_item ? 'specialist_id' and nullif(btrim(v_item->>'specialist_id'), '') is not null then
    begin
  ...
  if v_specialist_id is null and v_specialist_code is null then
    return jsonb_build_object('ok', false, 'error', 'specialist_code_or_id_required');
  end if;
  ...
  v_appointment_id := fn_cancel_appointment_by_booking_uid(
    v_tenant_id,
    v_booking_uid,
    'Cancelada por paciente desde WhatsApp (status cancelado)',
    'whatsapp',
    'patient',
    v_numero,
    null
  ...
  if v_appointment_id is not null then
    ...
    set metadata = coalesce(a.metadata, '{}'::jsonb)
      || jsonb_build_object(
        ...
        'botsailor_status', btrim(v_status_in)
      )
...
comment on function fn_botsailor_appointment_response(jsonb) is
  'Array [{numero, tenant_id, specialist_code|id, status: confirmado|cancelado}]. Sin status valido no modifica citas.';

revoke all on function fn_botsailor_appointment_response(jsonb) from public, anon, authenticated;
grant execute on function fn_botsailor_appointment_response(jsonb) to service_role;
